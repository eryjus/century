...
//  ----------  -------  -------  ----  ---------------------------------------------------------------------------
//  2017-05-25  Initial   0.0.0   ADCL  Initial version (second iteration)
//
//===================================================================================================================
...
//-------------------------------------------------------------------------------------------------------------------
// NewPdAddr() -- Obtain the frame address for the mmu and store it back in the data structure
//-------------------------------------------------------------------------------------------------------------------
static struct PageEntry *NewPdAddr(void)
{
...
#ifdef DEBUG_MMU
    kprintf(u8"MMU: The resulting PD frame is %p\n", mbLocal.mmu);
#endif
...
{
#ifdef DEBUG_MMU
    kprintf(u8"MMU: Mapping address 0x%08x to frame %p\n", addr, frame);
#endif
    ...
    if (pEntry->p) {
        kprintf(u8"MMU: Address 0x%08x is already mapped to frame %p\n", addr, pEntry->frame);
        return false;
    }
...
void MmuTablesInit(void)
{
    ...
    struct PageEntry *rEntry = &cr3[1023];
...
void MmuDumpTables(arch_addr_t addr)
{
    ...
    uint32_t i = (addr >> MMU_PD_BITS) & 0x3ff;
